...
def main():
    ...
    content=np.zeros(4)
    for i in range(4):
        content[i]= round(float(filehandle.readline().strip())*image_side/448)
    print(content)
    count=(content[1]-content[0])*(content[3]-content[2])*3
    while (count%16)!=0:
        content[0]-=1
        count = (content[1] - content[0]) * (content[3] - content[2]) * 3

    jiami=np.empty((content[3]-content[2],content[1]-content[0],3),dtype=np.uint8)
    print(jiami.shape)
    jiami[:,:,:]=npmat[content[2]:content[3],content[0]:content[1],:]
    print(jiami)
    xx=int(content[3]-content[2])
    yy=int(content[1]-content[0])
    n=np.zeros(xx*yy*3)
    m=np.zeros(64)
    mn=np.empty((xx,yy,3),dtype=np.uint8)
    pp = np.zeros(xx * yy * 3)
    index=0
    for i in range(xx):
        for j in range(yy):
    ...
    n=np.zeros(xx*yy*3)
    m=np.zeros(64)
    nn=np.empty((xx,yy,3),dtype=np.uint8)
    pp = np.zeros(xx * yy * 3)
    index=0
    for i in range(xx):
        for j in range(yy):
    ...
    result=np.empty((image_side,image_side,3),dtype=np.uint8)
    print(result.shape)
    print(mn.shape)
    print(content)
    print(image_side)
    result[content[2]:content[3],content[0]:content[1],:]=mn[:,:,:]
    result[0:content[2],:,:]=npmat[0:content[2],:,:]
    result[:,0:content[0],:]=npmat[:,0:content[0],:]
    result[:,content[1]:,:]=npmat[:,content[1]:,:]
    result[content[3]:,:,:]=npmat[content[3]:,:,:]
    image = Image.fromarray(result, 'YCbCr')
    image = image.convert('RGB')
    image.show()
    image.save('/home/zsa/PycharmProjects/tensorflow-yolo-python2.7/bird2.jpeg')
    print(content)
